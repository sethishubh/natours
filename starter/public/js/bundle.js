...
var displayMap = function displayMap(locations) {
  ...
  mapboxgl.accessToken = 'pk.eyJ1Ijoic2V0aGk2Nzc3IiwiYSI6ImNrYWx5dmR6eDB2aXYycm82ZHZ5bWhleWkifQ.eCwAI8Y3vHksXVmsTYvIFw'; //- Below's code is copied from installation steps given on mapbox website

  var map = new mapboxgl.Map({
    container: 'map',
    //So, this code will create a map and display it(map) on tour.pug/SpecificTourDetailTemplate page . Coz container here is set to 'map' and this means that it will put the map on an element with the ID (And Not class) of 'map' and so thats y in tour.pug file we have code-> #map(data-.....) -> ie element with ID('#') called 'map'
  ...
  var bounds = new mapboxgl.LngLatBounds(); //And we got access to the 'mapboxgl' object here coz we included the Mapbox library at the beginning of our page (ie on tour.pug in code-> "script(src= 'https://api.mapb....')"
  ...
  locations.forEach(function (loc) {
    ...
    var el = document.createElement('div'); //createElement('div') -> creating new element(div here)
    ...
    }).setLngLat(loc.coordinates).addTo(map); //Above calling -> setLngLat() to make Mapbox know about the GPS co-ordinates of this marker(pin) ; 'coordinates' in loc.coordinates above = array of long,lat in it = coordinates = [lng, lat] And also, in tours.json file-> tour.locations.coordinates = array of [lat, lng] ; map -> map variable created above in var map = ...
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, " </p>")).addTo(map); //loc-> current location/stop in loop iteration and so loc.day = currentLocation.day = locations.day in tours.json file
...
var showAlert = function showAlert(type, msg) {
  ...
  hideAlert();
  var markup = "<div class=\"alert alert--".concat(type, "\">").concat(msg, "</div>"); //Below-> document.querySelector('select the element where we want to include this HTML that we just created above in const markup = .... . So that's actually right at the beginning of the body element. So right at the top of the page. So body.')

  document.querySelector('body').insertAdjacentHTML('afterbegin', markup); //Above-> afterbegin -> means inside of the body, but right at the beginning. ; markup -> HTML we want to include
        ...
        switch (_context.prev = _context.next) {
          case 0:
            ...
            return (0, _axios.default)({
              ...
              // endpoint(login endpoint here) for API req
              //Below is the data that we're sending along with the request in the body(like in the postman body in a POST req)
              data: {
                email: email,
                // LHS 'email': coz our login endpoint/API expects our body data to be called as -> {"email":"max@example.com", "password": 12345} in the body of postman in login endpoint and LHS email ->is a property ; RHS email is the parameter/argument of this f'n (ie in code -> 'const login = (email, password) ....' above) AND exact same case is for p/w defined below
                password: password
              }
            });
          ...
          case 4:
            ...
            if (res.data.status === 'success') {
              ...
              (0, _alerts.showAlert)('success', 'Logged-in successfully'); //And now after one and a half seconds(1500 ms) of successfully logging-in on our login form page/webpage on website, load the front page. So basically, the home page below (ie '/' page/webpage of our website)
          ...
          case 9:
            ...
            (0, _alerts.showAlert)('error', _context.t0.response.data.message); // data in err.response.data is data in/on response. So, 'data' here = data response = JSON WE RECEIVED AS THE RESPONSE FROM(AFTER SUCCESSFUL CALL TO ABOVE '/login' API) '/login' API as a result of successfull call to above '/login' API ; message = message property on/in received json response from '/login' API
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            url = type === 'password' ? 'http://127.0.0.1:3000/api/v1/users/updateMyPassword' : 'http://127.0.0.1:3000/api/v1/users/updateMe'; //res below -> response
          ...
          case 8:
            ...
            (0, _alerts.showAlert)('error', _context.t0.response.data.message); //message in 'err.response.data.message' is the property that we are defining on the server(ie in our API Response) whenever there is an error
...
var stripe = Stripe('pk_test_51GreqyEFgSFq07bvntETD1nmy6HKj8DtsLeI0xbigeYbKL8NdpcqjLqnoDFex256PDl4Wjim08GJIX60JctON2mW00lbiDkFOR');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            console.log('session'); // 2) Use stripe object 2 automatically create d checkout form + charge credit card
...
//Contd..
console.log('hello from parcel'); //Below -> So this one here below we do not save it into any variable, because that's not necessary at all. All we want this to do is to basically be included into our final bundle to well polyfill some of the features of JavaScript. and so that's also why it's here in the first line of these imports.
//

//Below-> DOM ELEMENTS
var mapBox = document.getElementById('map');
var loginForm = document.querySelector('.login-form');
var logOutBtn = document.querySelector('.nav__el--logout'); //.nav__el--logout = class in __header.pug file

var userDataForm = document.querySelector('.form-user-data'); //from account.pug file

var userPasswordForm = document.querySelector('.form-user-password'); //from account.pug file

var bookBtn = document.getElementById('book-tour'); //Below-> DELEGATION

if (mapBox) {
  //For just below's code explanation, go to mapbox.js
  var locations = JSON.parse(mapBox.dataset.locations); //Calling imported displayMap f'n here below with passing above locations as parameters into it
...
if (loginForm) {
  ...
  loginForm.addEventListener('submit', function (e) {
    e.preventDefault(); //below getting email and the p/w value that the user puts in login form(in login.pug file) with the id email (ie #email) and id p/w (ie #password)
    // const email = document.getElementById('email').value;
    // const password = document.getElementById('password').value;
    //VALUES Below

    var email = document.getElementById('email').value;
    var password = document.getElementById('password').value; //calling login() f'n below
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    e.preventDefault(); //Below creating multipart/form-data
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              passwordCurrent = document.getElementById('password-current').value; //from accoun.pug file

              password = document.getElementById('password').value; //from accoun.pug file

              passwordConfirm = document.getElementById('password-confirm').value; //from accoun.pug file
              //updateData(name, email);
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (e) {
    e.target.textContent = 'Processing...'; //e.target = the element(Button here) vch was clicked ie so d one dat triggered dis event listener above 2 b fired
